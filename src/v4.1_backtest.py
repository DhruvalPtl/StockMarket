...
class NiftyV4_1:
    def __init__(self):
        try:
            ...
            self.init_logs()
            self.load_expiries()
            print(">>> V4.1 Initialized. Logic Fixed.")
    ...
    def run_backtest(self):
        ...
        in_pos = False
        print(f"2. Data Good. Starting Loop on {len(df)} candles...")
        print("-" * 60)
        ...
        for i in range(13, len(df)):
            ...
            if not active_expiry:
                self.log_mov(clean_time, row, "UNKNOWN", "N/A", 0, 0, "SKIP", "Expiry Unknown")
                continue 
            ...
            if not in_pos and trend_up and trigger and short_covering:
                print(f"\n[!] Setup at {clean_time}. Hunting Budget Strike...")
                ...
                for sym in candidates:
                    ...
                    t_start = row['time']
                    try:
                        dt = datetime.strptime(str(row['time']).split('+')[0].replace('T', ' '), "%Y-%m-%d %H:%M:%S")
                    except:
                    ...
                    if opt_df is not None:
                        ...
                        if prem < MAX_PREMIUM:
                            ...
                            # --- FIX: Reset Loop State ---
                            in_pos = False  # Allows next trade!
                            found = True
                            break # Trade done, stop hunting
                        else:
            ...
            # SCANNING LOG
                ...
                reason = "Waiting"
                if not short_covering: reason = "No Short Covering"
                elif not trend_up: reason = "No Trend"
                self.log_mov(clean_time, row, active_expiry, "N/A", 0, oi_chg, "SCAN", reason)

        print("\n" + "-" * 60)
        print("Backtest Complete. Check V4_BOT_MOVEMENT.csv")
