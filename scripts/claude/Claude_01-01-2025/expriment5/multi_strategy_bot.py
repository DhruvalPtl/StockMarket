...
from strategies import BaseStrategy, LiquiditySweepStrategy
...
class StrategyRunner:
    ...
    def _exit_position(self, exit_price: float, reason: str):
        """
        Closes the active trade and logs the result with REALISM (Costs deducted).
        """
        ...
        # --- 1. CALCULATE GROSS PnL (Paper Profit) ---
        gross_pnl = (exit_price - pos['entry_price']) * pos['qty']
        
        # --- 2. APPLY REALISM FILTER (The Costs) ---
        # Brokerage: Buy + Sell
        brokerage = BotConfig.BROKERAGE_PER_ORDER * 2
        # Taxes: STT, GST, Stamp Duty
        taxes = BotConfig.TAXES_PER_TRADE
        # Slippage: Points lost per quantity
        slippage_cost = BotConfig.SLIPPAGE_POINTS * pos['qty']
        
        total_costs = brokerage + taxes + slippage_cost
        
        # --- 3. CALCULATE NET PnL (Real Profit) ---
        net_pnl = gross_pnl - total_costs
        
        # Percent Calculation based on Entry Capital
        invested_amount = pos['entry_price'] * pos['qty']
        pnl_pct = (net_pnl / invested_amount) * 100 if invested_amount > 0 else 0.0
        ...
        trade_record = {
            ...
            "Balance": round(self.current_capital, 2),
            "Exit_Reason": reason
        }
        
        self.trades_today.append(trade_record)
        
        # Log to CSV (Logger handles printing)
    ...
    def get_summary(self) -> Dict:
        ...
        wins = len([t for t in self.trades_today if t.get('PnL', 0) > 0])
        return {
