...
=====================
Data pipeline that works with both Groww and Flate Trade APIs.
Port of claude_groww_data_pipeline.py using UnifiedAPI.

Features:
- Fetch spot prices, futures, RSI, EMA, VWAP
- Fetch option chain, Greeks (Delta, Gamma, Theta, Vega), PCR
- Log to CSV in same format as Groww pipeline
- Command-line argument to switch between providers

Usage:
    # Use Groww (default)
    python data_pipeline.py --api groww
    
    # Use Flate Trade
    python data_pipeline.py --api flate
...
class UnifiedDataEngine:
    """
    Data engine that works with both Groww and Flate Trade APIs.
    
    Usage:
        engine = UnifiedDataEngine(
            provider="groww",  # or "flate"
            api_key=key,       # for Groww
            api_secret=secret, # for Groww
    ...
    def __init__(self, provider, expiry_date, fut_symbol, **api_credentials):
        print(f"\n⚙️ STARTING UNIFIED ENGINE v1.0")
        ...
        self.log_file = os.path.join(
            log_dir,
            f"Unified_{provider.upper()}_Log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        self._init_csv()
    ...
    def _connect(self, **api_credentials):
        """Connect to the selected API provider"""
        try:
    ...
    def _fetch_chain(self):
        """Fetch option chain and extract Greeks"""
        try:
            chain = self.api.get_option_chain("NSE", "NIFTY", self.expiry_date)
            ...
            # Save the raw chain data for symbol lookup
            self.chain_data = chain.get('strikes', {})
            ...
            # Process chain data
            for strike_str, data in chain.get('strikes', {}).items():
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expiry', type=str, default=BotConfig.OPTION_EXPIRY,
                       help='Option expiry date (YYYY-MM-DD)')
    parser.add_argument('--updates', type=int, default=10,
                       help='Number of updates to fetch (default: 10)')
    
    args = parser.parse_args()
    
    print(f"🧪 Testing Unified Data Pipeline with {args.api.upper()} API\n")
    ...
    for i in range(args.updates):
        print(f"\n--- Update {i+1}/{args.updates} ---")
        engine.update()
        
        if i < args.updates - 1:  # Don't sleep on last iteration
            time_module.sleep(30)  # 30 seconds between updates
    ...
    print(f"\n✅ Data logged to: {engine.log_file}")
